...
authenticateAgent();`;
  ...
  return (
    <div className="space-y-8 font-tajawal">
      {/* Hero Header */}
      <div className="p-8 rounded-[var(--pub-radius-container)] border border-[var(--pub-border-default)] bg-[var(--pub-surface-panel)] relative overflow-hidden transition-theme">
        <div className="absolute top-0 right-0 p-8 opacity-5 text-cyan-400 pointer-events-none">
          <Terminal size={140} />
        </div>
        <div className="relative z-10 max-w-3xl space-y-3">
          <div className="inline-flex items-center gap-2 px-3 py-1 rounded-[var(--pub-radius-micro)] bg-cyan-500/10 border border-cyan-500/20 text-cyan-400 text-[11px] font-bold tracking-wider uppercase">
            <Cpu size={12} />
            {isAr ? 'بروتوكول الوكلاء والأتمتة' : 'AI Bot & Agent Integration Profile'}
          </div>
          <h2 className="text-2xl font-black tracking-tight text-[var(--pub-text-primary)]">
            {isAr ? 'بوابة المطورين وتكامل الأنظمة الخارجية' : 'Developer & External Agents Portal'}
          </h2>
          <p className="text-sm text-[var(--pub-text-secondary)] leading-relaxed md:w-[85%]">
            {isAr 
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
        ...
        <div className="lg:col-span-5 space-y-6">
          <div className="p-6 md:p-8 rounded-[var(--pub-radius-container)] border border-[var(--pub-border-default)] bg-[var(--pub-surface-panel)] space-y-6 transition-theme">
            <div className="flex items-center gap-2 pb-4 border-b border-[var(--pub-border-default)]">
              <Key size={18} className="text-cyan-400" />
              <h3 className="font-bold text-base text-[var(--pub-text-primary)]">
            ...
            <form onSubmit={handleRegisterAgent} className="space-y-4">
              ...
              <div className="space-y-1">
                ...
                />
                <span className="text-[10px] text-[var(--pub-text-muted)] leading-relaxed block select-none">
              ...
              <button
                ...
                className="w-full flex items-center justify-center gap-2 py-3 px-4 bg-cyan-500 hover:bg-cyan-400 text-black font-bold text-sm tracking-tight rounded-[var(--pub-radius-control)] shadow-sm transition-theme disabled:opacity-50 cursor-pointer"
              >
                {isRegistering ? (
        ...
        <div className="lg:col-span-7 space-y-6">
          ...
          {/* Active Agents List */}
          <div className="p-6 md:p-8 rounded-[var(--pub-radius-container)] border border-[var(--pub-border-default)] bg-[var(--pub-surface-panel)] space-y-4 transition-theme">
            <div className="flex items-center justify-between pb-4 border-b border-[var(--pub-border-default)]">
              ...
              <span className="text-xs px-2.5 py-0.5 rounded-[var(--pub-radius-micro)] bg-cyan-500/10 text-cyan-400 border border-cyan-500/20 font-bold">
                {agents.length}
              </span>
            </div>
            ...
            {isLoading ? (
              ...
              <div className="h-32 flex flex-col items-center justify-center text-center space-y-2">
                <span className="text-xs text-[var(--pub-text-muted)] block select-none">
                  {isAr ? 'لا يوجد لديك أي وكلاء مسجلين حالياً.' : 'No automated client connections configured yet.'}
                </span>
                <span className="text-[10px] text-[var(--pub-text-muted)] block">
                  {isAr ? 'املأ النموذج على الجانب لإنشاء شريك توثيق خارجي جديد.' : 'Create credentials to start integrating other servers.'}
                </span>
              </div>
            ) : (
              <div className="divide-y divide-[var(--pub-border-default)] max-h-56 overflow-y-auto no-scrollbar">
                {agents.map(agent => (
                  <div key={agent.id} className="py-3.5 flex items-center justify-between gap-4">
                    <div className="space-y-1">
                      <div className="flex items-center gap-2">
                        <span className="text-sm font-bold text-[var(--pub-text-primary)]">
                          {agent.client_name}
                        </span>
                        <span className="text-[9px] px-1.5 py-0.5 rounded-[var(--pub-radius-micro)] bg-[var(--pub-surface-subtle)] border border-[var(--pub-border-default)] text-[var(--pub-text-muted)] font-mono tracking-wide uppercase">
                          {agent.identity_type}
                        </span>
                      </div>
                      <div className="flex items-center gap-1 font-mono text-[10px] text-[var(--pub-text-muted)]">
                        <span>ID:</span>
                        <span className="text-cyan-400">{agent.client_id}</span>
                        <button 
                          onClick={() => handleCopy(agent.client_id, `cid-${agent.id}`)}
                          className="p-1 hover:text-[var(--pub-text-primary)] transition-colors cursor-pointer"
                        >
                    ...
                    <div className="flex items-center gap-3">
                      <span className="text-[10px] text-[var(--pub-text-muted)]">
                        {new Date(agent.created_at).toLocaleDateString(isAr ? 'ar-EG' : 'en-US')}
                      </span>
                      <button
                        onClick={() => handleRevokeAgent(agent.client_id)}
                        className="w-8 h-8 flex items-center justify-center rounded-[var(--pub-radius-control)] bg-transparent hover:bg-rose-500/10 text-[var(--pub-text-muted)] hover:text-rose-500 transition-theme border border-transparent hover:border-rose-500/20 cursor-pointer"
          ...
          <AnimatePresence>
            {generatedCredentials && (
              <motion.div
                ...
                className="p-6 md:p-8 rounded-[var(--pub-radius-container)] border border-cyan-500/30 bg-cyan-500/[0.03] space-y-4 overflow-hidden"
              >
                <div className="flex items-start gap-3">
                  <div className="p-2 rounded-[var(--pub-radius-control)] bg-cyan-500/10 text-cyan-400 border border-cyan-500/20 shrink-0">
                    <ShieldAlert size={18} />
                  </div>
                  <div className="space-y-1">
                    <h4 className="font-bold text-sm text-[var(--pub-text-primary)]">
                      {isAr ? 'هام للغاية: انسخ مفاتيح التوثيق الخاصة بالوكيل المولد!' : 'CRITICAL Security Credentials Generated!'}
                    </h4>
                    <p className="text-xs text-[var(--pub-text-secondary)] leading-relaxed">
                      {isAr 
                ...
                <div className="space-y-3 font-mono text-xs p-4 bg-[var(--pub-surface-subtle)] rounded-[var(--pub-radius-control)] border border-[var(--pub-border-default)]">
                  <div className="flex items-center justify-between gap-4">
                    <span className="text-[var(--pub-text-muted)]">Client Name:</span>
                    <span className="text-[var(--pub-text-primary)] font-bold">{generatedCredentials.client_name}</span>
                  </div>
                  <div className="flex items-center justify-between gap-4 py-1.5 border-t border-[var(--pub-border-default)]">
                    <span className="text-[var(--pub-text-muted)]">Client ID:</span>
                  ...
                  <div className="flex items-center justify-between gap-4 py-1.5 border-t border-[var(--pub-border-default)]">
                    <span className="text-[var(--pub-text-muted)]">Client Secret:</span>
                    <div className="flex items-center gap-1 text-[var(--pub-text-primary)] font-bold select-all">
          ...
          <div className="p-6 md:p-8 rounded-[var(--pub-radius-container)] border border-[var(--pub-border-default)] bg-[var(--pub-surface-panel)] space-y-4 transition-theme">
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 pb-2">
              ...
              {/* Code language tabs */}
              <div className="flex items-center gap-1 bg-[var(--pub-surface-subtle)] p-0.5 rounded-[var(--pub-radius-control)] self-start border border-[var(--pub-border-default)]">
                <button
                  onClick={() => setCodeLanguage('python')}
                  className={`text-[10px] font-bold px-2.5 py-1 rounded-[var(--pub-radius-micro)] transition-colors cursor-pointer ${codeLanguage === 'python' ? 'bg-cyan-500 text-black' : 'text-[var(--pub-text-muted)] hover:text-[var(--pub-text-primary)]'}`}
                >
                  Python
                </button>
                <button
                  onClick={() => setCodeLanguage('node')}
                  className={`text-[10px] font-bold px-2.5 py-1 rounded-[var(--pub-radius-micro)] transition-colors cursor-pointer ${codeLanguage === 'node' ? 'bg-cyan-500 text-black' : 'text-[var(--pub-text-muted)] hover:text-[var(--pub-text-primary)]'}`}
                >
                  NodeJS
                </button>
                <button
                  onClick={() => setCodeLanguage('curl')}
                  className={`text-[10px] font-bold px-2.5 py-1 rounded-[var(--pub-radius-micro)] transition-colors cursor-pointer ${codeLanguage === 'curl' ? 'bg-cyan-500 text-black' : 'text-[var(--pub-text-muted)] hover:text-[var(--pub-text-primary)]'}`}
                >
                  cURL
                </button>
              </div>
            ...
            <div className="relative">
              <button
                ...
                className="absolute top-3 right-3 p-1.5 rounded-[var(--pub-radius-micro)] bg-[var(--pub-surface-panel)] hover:bg-[var(--pub-surface-container)] border border-[var(--pub-border-default)] text-[var(--pub-text-muted)] hover:text-[var(--pub-text-primary)] transition-theme cursor-pointer"
                title={isAr ? 'نسخ الكود' : 'Copy Code'}
              >
                {copiedId === 'code-copy' ? <Check size={12} className="text-cyan-400" /> : <Copy size={12} />}
              </button>
              <pre className="text-[10px] font-mono leading-relaxed p-4 bg-[var(--pub-surface-subtle)] text-[var(--pub-text-secondary)] rounded-[var(--pub-radius-control)] overflow-x-auto max-h-52 no-scrollbar border border-[var(--pub-border-default)]">
                {getCodeSnippet()}
              </pre>
            </div>

            <div className="flex items-center gap-2 pt-2 text-[11px] text-[var(--pub-text-muted)]">
              <BookOpen size={12} />
              <span>
                {isAr 
          ...
          <div className="p-6 md:p-8 rounded-[var(--pub-radius-container)] border border-[var(--pub-border-default)] bg-[var(--pub-surface-panel)] space-y-6 transition-theme">
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 pb-2 border-b border-[var(--pub-border-default)]">
              <div className="flex items-center gap-2">
                <div className="relative">
                  <Cpu size={20} className="text-cyan-400 animate-pulse" />
                  <div className="absolute -inset-1 rounded-[var(--pub-radius-control)] bg-cyan-500/20 blur opacity-75"></div>
                </div>
                <div>
                  <h4 className="font-bold text-sm text-[var(--pub-text-primary)] flex items-center gap-2">
                    {isAr ? 'مركز عمليات بروتوكول WebMCP المتكامل' : 'WebMCP Integrated Protocol Workspace'}
                    <span className="px-1.5 py-0.5 rounded-[var(--pub-radius-micro)] bg-cyan-500/10 text-cyan-400 border border-cyan-500/20 text-[9px] font-mono font-bold tracking-widest uppercase">
                      ACTIVE
                    </span>
                  </h4>
                  <p className="text-[10px] text-[var(--pub-text-muted)]">
              ...
              <div className="flex items-center gap-1 bg-[var(--pub-surface-subtle)] p-0.5 rounded-[var(--pub-radius-control)] border border-[var(--pub-border-default)] self-start">
                <button
                  type="button"
                  onClick={() => setMcpMode('server')}
                  className={`text-[10px] font-bold px-3 py-1 rounded-[var(--pub-radius-micro)] cursor-pointer transition-theme ${mcpMode === 'server' ? 'bg-cyan-500 text-black shadow-sm' : 'text-[var(--pub-text-muted)] hover:text-[var(--pub-text-primary)]'}`}
                >
                ...
                <button
                  ...
                  className={`text-[10px] font-bold px-3 py-1 rounded-[var(--pub-radius-micro)] cursor-pointer transition-theme ${mcpMode === 'federation' ? 'bg-cyan-500 text-black shadow-sm' : 'text-[var(--pub-text-muted)] hover:text-[var(--pub-text-primary)]'}`}
                >
              ...
              <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
                ...
                <div className="lg:col-span-6 space-y-4">
                  <div className="space-y-2">
                    <span className="text-xs font-bold text-[var(--pub-text-primary)] flex items-center gap-1">
                      <Activity size={14} className="text-cyan-400" />
                      {isAr ? 'أدوات التحكم وتدفق الاكتشاف:' : 'Discovery Flow controls:'}
                    </span>
                    <p className="text-[11px] text-[var(--pub-text-secondary)] leading-relaxed">
                      {isAr 
                  ...
                  <div className="flex flex-wrap gap-2">
                    <button
                      ...
                      className="text-xs font-bold px-4 py-2 bg-[var(--pub-surface-subtle)] hover:bg-[var(--pub-surface-container)] rounded-[var(--pub-radius-control)] border border-[var(--pub-border-default)] text-cyan-400 hover:text-cyan-300 flex items-center gap-1 transition-theme cursor-pointer disabled:opacity-50"
                    >
                  ...
                  {mcpDiscovery && (
                    <motion.div
                      ...
                      className="p-3 bg-[var(--pub-surface-subtle)] rounded-[var(--pub-radius-control)] border border-cyan-500/20 space-y-1"
                    >
                      <span className="text-[10px] text-cyan-400 font-bold font-mono uppercase tracking-wider block">
                        [LIVE DISCOVERY META]
                      </span>
                      <div className="text-[11px] font-mono text-[var(--pub-text-secondary)] space-y-1">
                        <div><span className="text-[var(--pub-text-muted)]">Service:</span> {mcpDiscovery.serverInfo?.name}</div>
                        <div><span className="text-[var(--pub-text-muted)]">Version:</span> {mcpDiscovery.serverInfo?.version}</div>
                        <div><span className="text-[var(--pub-text-muted)]">Protocol:</span> {mcpDiscovery.supportedProtocolVersions?.join(', ')}</div>
                        <div><span className="text-[var(--pub-text-muted)]">Transport:</span> {mcpDiscovery.transport?.type} ({mcpDiscovery.transport?.endpoint})</div>
                      </div>
                    </motion.div>
                  )}
                  ...
                  {mcpTools.length > 0 && (
                    <motion.div
                      initial={{ opacity: 0, y: 5 }}
                      animate={{ opacity: 1, y: 0 }}
                      className="p-4 rounded-[var(--pub-radius-control)] border border-[var(--pub-border-default)] bg-[var(--pub-surface-subtle)] space-y-4"
                    >
                      <div className="space-y-1">
                        <span className="text-xs font-bold text-[var(--pub-text-primary)] flex items-center gap-1">
                      ...
                      <div className="space-y-2">
                        ...
                        <div className="space-y-1">
                          <label className="text-[10px] text-[var(--pub-text-muted)] font-bold block">{isAr ? 'معلمة الإدخال (prompt / query / input):' : 'Input Argument (prompt / query / input):'}</label>
                          <div className="flex gap-2">
                            <input
                              ...
                              className="flex-1 text-xs bg-[var(--pub-surface-panel)] border border-[var(--pub-border-default)] rounded-[var(--pub-radius-control)] p-2 text-[var(--pub-text-primary)] focus:border-cyan-500/60 focus:outline-none transition-colors"
                            />
                            <button
                              type="button"
                              onClick={handleExecuteMcpTool}
                              disabled={isMcpWorking || !mcpSelectedTool}
                              className="px-3.5 bg-cyan-500 hover:bg-cyan-400 disabled:opacity-50 text-black font-bold text-xs rounded-[var(--pub-radius-control)] flex items-center justify-center transition-theme cursor-pointer"
                            >
                      ...
                      {mcpExecutionResult && (
                        <div className="space-y-1">
                          ...
                          <pre className="text-[10px] font-mono leading-relaxed p-3 bg-[var(--pub-surface-panel)] text-[var(--pub-text-secondary)] rounded-[var(--pub-radius-control)] border border-cyan-500/20 overflow-x-auto max-h-40 no-scrollbar select-all">
                            {mcpExecutionResult}
                          </pre>
                        </div>
                      )}
                ...
                <div className="lg:col-span-6 space-y-2">
                  <div className="flex items-center justify-between">
                    ...
                    <button
                      type="button"
                      onClick={() => setMcpLogs([])}
                      className="text-[10px] text-[var(--pub-text-muted)] hover:text-[var(--pub-text-primary)] transition-colors cursor-pointer"
                    >
                      {isAr ? 'مسح السجلات' : 'Clear logs'}
                    </button>
                  </div>

                  <div className="h-56 bg-[var(--pub-surface-subtle)] p-4 rounded-[var(--pub-radius-control)] border border-[var(--pub-border-default)] font-mono text-[10px] text-cyan-400 overflow-y-auto no-scrollbar space-y-2 flex flex-col-reverse shadow-inner">
                    {mcpLogs.length === 0 ? (
                      <span className="text-[10px] text-[var(--pub-text-muted)] block uppercase tracking-wider select-none h-full flex items-center justify-center text-center">
                  ...
                  <div className="p-3 bg-cyan-500/[0.03] rounded-[var(--pub-radius-control)] border border-cyan-500/10 text-[10px] text-[var(--pub-text-muted)] leading-relaxed">
                    {isAr 
              ...
              <div className="space-y-4">
                ...
                <form onSubmit={handleConnectExternalMcp} className="flex gap-2 max-w-xl">
                  <div className="flex-1 relative">
                    <input
                      ...
                      className="w-full text-xs bg-[var(--pub-surface-subtle)] border border-[var(--pub-border-default)] rounded-[var(--pub-radius-control)] p-2.5 text-[var(--pub-text-primary)] focus:border-cyan-500/60 focus:outline-none transition-theme"
                      required
                    />
                  ...
                  <button
                    type="submit"
                    disabled={isMcpWorking}
                    className="px-4 bg-cyan-500 hover:bg-cyan-400 text-black font-bold text-xs rounded-[var(--pub-radius-control)] flex items-center gap-1 transition-theme cursor-pointer disabled:opacity-50 shrink-0"
                  >
                ...
                {externalMcpStatus === 'connected' && (
                  <motion.div
                    ...
                    className="p-4 rounded-[var(--pub-radius-control)] border border-cyan-500/20 bg-cyan-500/[0.03] space-y-3 max-w-xl"
                  >
                    <div className="flex items-center gap-2">
                    ...
                    <div className="text-[11px] text-[var(--pub-text-secondary)] space-y-1 font-mono">
                      <div className="flex justify-between border-b border-[var(--pub-border-default)] pb-1">
                        <span>{isAr ? 'معرف الاتصال:' : 'Federated ID:'}</span>
                        <span className="text-[var(--pub-text-primary)]">fed_client_e9821a</span>
                      </div>
                      <div className="flex justify-between border-b border-[var(--pub-border-default)] py-1">
          ...
          <div className="p-6 md:p-8 rounded-[var(--pub-radius-container)] border border-[var(--pub-border-default)] bg-[var(--pub-surface-panel)] space-y-4 transition-theme">
            <div className="flex items-center gap-2 pb-2 border-b border-[var(--pub-border-default)]">
              <Globe size={18} className="text-cyan-400" />
              <h4 className="font-bold text-sm text-[var(--pub-text-primary)]">
            ...
            <div className="space-y-4">
              ...
              <div className="space-y-2">
                ...
                <div className="relative">
                  <button
                    ...
                    className="absolute top-2 right-2 p-1 rounded-[var(--pub-radius-micro)] bg-[var(--pub-surface-panel)] hover:bg-[var(--pub-surface-container)] border border-[var(--pub-border-default)] text-[var(--pub-text-muted)] hover:text-[var(--pub-text-primary)] transition-theme text-[10px] cursor-pointer"
                    title={isAr ? 'نسخ السجل' : 'Copy DNS Record'}
                  >
                    {copiedId === 'dns-copy' ? <Check size={10} className="text-cyan-400" /> : <Copy size={10} />}
                  </button>
                  <pre className="text-[10px] font-mono leading-relaxed p-3 bg-[var(--pub-surface-subtle)] text-[var(--pub-text-secondary)] rounded-[var(--pub-radius-control)] overflow-x-auto no-scrollbar border border-[var(--pub-border-default)]">
                ...
                <div className="text-[10px] text-[var(--pub-text-muted)] leading-relaxed block pl-1">
                  {isAr 
              ...
              {/* Link Headers block */}
              <div className="space-y-2 pt-2 border-t border-[var(--pub-border-default)]">
                <span className="text-[11px] font-bold text-cyan-400">
                ...
                <pre className="text-[10px] font-mono leading-relaxed p-3 bg-[var(--pub-surface-subtle)] text-[var(--pub-text-muted)] rounded-[var(--pub-radius-control)] overflow-x-auto no-scrollbar border border-[var(--pub-border-default)] select-all">
          ...
          <div className="p-6 md:p-8 rounded-[var(--pub-radius-container)] border border-[var(--pub-border-default)] bg-[var(--pub-surface-panel)] space-y-4 transition-theme">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 pt-2">
              <div className="p-4 bg-[var(--pub-surface-subtle)] rounded-[var(--pub-radius-control)] border border-[var(--pub-border-default)] space-y-2 font-mono text-xs">
                <span className="text-cyan-400 font-bold block mb-1">
                  {isAr ? '⚙️ طرف التوصيل المحمي برمجياً:' : '⚙️ Protected Resource Endpoint:'}
                </span>
                <div className="text-[var(--pub-text-primary)] bg-[var(--pub-surface-panel)] p-2 rounded-[var(--pub-radius-micro)] text-[11px] select-all border border-[var(--pub-border-default)]">
                  {`GET /api/agent/exclusive-analysis`}
                </div>
                <div className="pt-2 text-[var(--pub-text-muted)] text-[10px] space-y-1">
                  <div className="flex justify-between">
                    <span>{isAr ? 'بروتوكول السداد:' : 'Payment Scheme:'}</span>
                    <span className="text-[var(--pub-text-primary)] font-bold">exact</span>
                  </div>
                  <div className="flex justify-between">
                    <span>{isAr ? 'شبكة العمل المقترحة:' : 'Invoiced Network:'}</span>
                    <span className="text-[var(--pub-text-primary)] font-bold">Base Sepolia</span>
                  </div>
                  <div className="flex justify-between">
                  ...
                  <div className="flex justify-between">
                    ...
                    <span className="text-[var(--pub-text-primary)] font-bold">0.10 USDC</span>
                  </div>
                </div>
              </div>

              <div className="p-4 bg-[var(--pub-surface-subtle)] rounded-[var(--pub-radius-control)] border border-[var(--pub-border-default)] space-y-2">
                ...
                <pre className="text-[9px] font-mono leading-relaxed p-2.5 bg-[var(--pub-surface-panel)] text-[var(--pub-text-secondary)] rounded-[var(--pub-radius-micro)] overflow-x-auto no-scrollbar border border-[var(--pub-border-default)]">
